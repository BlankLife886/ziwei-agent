...
部署前可以运行 `npm run validate:deploy`。它会复用运行时配置门禁，检查已配置知识库是否通过审计，并临时启动同一个 HTTP server 请求 `/health` 与 `/v1/reports`，确认入口到用户报告发布链路可用。
