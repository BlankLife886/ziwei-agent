...
- `src/agent/interpretationCatalog.js`: 定义当前可用的命理解释条目，包括宫位角色、星曜角色、四化类型边界和核心宫位四化落宫边界，让报告正文引用受控解释，而不是直接散写断语。
