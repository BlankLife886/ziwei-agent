...
当前阶段实现了出生资料采集、历法转换、命宫/身宫计算、五行局计算、十四主星基础落宫，左辅、右弼、天刑、天姚、天月、天巫等月系辅星，三台、八座等日系辅星，禄存、擎羊、陀罗、天魁、天钺、天官、天福、天厨等年干星曜，截空等空曜，火星、铃星等基础煞曜，生年四化，大限年龄段，以及按分析日期定位当前大限。排盘流程已经抽成可复用的 `chartBuilder` 服务，并加入命理师 agent 外壳，用“分析上下文 -> 报告规划 -> 正文草稿”的流程组织命盘证据、分析重点和当前能力边界。婚姻感情方向已支持夫妻宫三方四正静态底稿，但仍不能推结婚时间、分合事件或伴侣具体身份。后续会在此基础上加入资料检索、命理分析和报告生成。
...
- `src/fourTransformationCalculator.js`: 根据出生年干计算生年四化，并把四化挂回对应星曜所在宫位。
