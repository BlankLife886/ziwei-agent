...
当前阶段实现了出生资料采集、历法转换、命宫/身宫计算、五行局计算、十四主星基础落宫，左辅、右弼、天刑、天姚、天月、天巫等月系辅星，三台、八座等日系辅星，禄存、擎羊、陀罗、天魁、天钺、天官、天福、天厨等年干星曜，截空等空曜，火星、铃星等基础煞曜，生年四化，大限年龄段，按分析日期定位当前大限、流年和流月，并能基于大限、流年、流月和四化重叠生成安全触发观察点、组合验证主题、组合主题解释、跨宫跨限运关系和报告专题细分任务单。排盘流程已经抽成可复用的 `chartBuilder` 服务，并加入命理师 agent 外壳，用“分析上下文 -> 报告规划 -> 报告生成器合同 -> provider 选择 -> 正文草稿 -> 报告审计 -> 用户报告发布”的流程组织命盘证据、分析重点、当前能力边界和输出安全检查。解释目录已为性格画像补入命宫十四主星及常见辅煞的保守解释，并为化禄、化权、化科、化忌、四化彼此合参、具体星曜被四化标记的角色边界、星曜四化同宫合参基础边界、十二宫四化落宫，以及婚姻、财富、事业三类专题宫位见四化时的合参边界补入受控结构解释；专题章节在本宫被生年四化、当前大限四化或流年四化命中时会追加对应证据，让命宫三方四正、生年四化、专题章节和当前阶段报告能引用受控条目而不是散写断语。报告生成器已支持同步/异步 provider，并提供通用外部 HTTP 大模型 provider 适配器；该适配器带有超时、重试、响应大小限制和脱敏诊断，外部模型产出的草稿仍必须通过报告审计和发布门禁。HTTP API 入口已加入请求追踪、结构化观测、请求大小限制、多凭证 scoped bearer 鉴权、credential 托管密钥命令桥接、secret 文件载入、内存限流和可选文件持久化配额，并提供轻量 Web UI 展示生辰输入、命盘图和审计后报告。事业、财富、婚姻和当前阶段运势已支持结构性底稿，但仍不能推具体年份事件、月份事件、应期、吉凶、婚恋结果、职位结果或财富结果。后续会在此基础上加入资料检索、命理分析和完整报告生成。
...
`npm start` 会加载 `data/knowledge-snippets.example.json`，用于演示“verified 知识片段 -> 报告规划 -> 知识覆盖审计 -> 用户报告引用”的闭环。`npm run report:markdown` 会运行同一条 agent pipeline，并且只有 `reportOutput` 已通过审计和发布门禁时才写出包含命盘图、正文和可追溯附录的 Markdown 用户报告；它不是绕过审计的单独生成器。`npm run validate:knowledge` 会同时输出 schema 校验和全局覆盖审计，按来源、主题和规则引用统计当前知识库是否足以支撑主要报告领域。这个示例知识库目前包含本地审校分析框架样本和专题知识笔记，覆盖命宫、性格、婚姻、财富、事业和当前阶段运势；它不代表用户提供的 PDF、书籍和扫描件已经完成全量结构化录入。
...
curl -X POST http://localhost:3000/v1/reports \
  ...
  -d '{"profile":{"name":"示例命主","gender":"female","calendar":"solar","birth_date":"1990-05-18","birth_time":"23:30","birth_place":"Shanghai, China","timezone":"Asia/Shanghai","use_true_solar_time":false,"is_leap_month":false,"analysis_date":"2026-07-01"},"query":"我想看婚姻和今年运势","outputFormats":["markdown"]}'
```

API 入口同样只调用统一 agent pipeline；它不会绕过报告规划、报告审计或发布门禁。`outputFormats:["markdown"]` 只是在报告已发布时额外返回 `artifacts.markdown`，内容与 CLI Markdown 导出同源，包含命盘图、正文和可追溯附录；如果报告被审计或人工确认阻断，则不会生成 Markdown 交付物。
...
`GET /openapi.json` 返回 OpenAPI 3.1 合同，覆盖 `/health`、`/ready` 和 `POST /v1/reports`，包括可选的 Markdown artifact 输出合同。它用于外部调用方、前端和部署检查对齐当前真实 API 边界，不声明尚未实现的报告能力。
...
- `src/agent/knowledgeStoreCoverageAuditor.js`: 对整个知识库按来源、topic、reference 和 riskLevel 做全局覆盖审计，避免只凭单条片段通过就误判为知识库充分。
...
- `src/agent/interpretationCatalog.js`: 定义当前可用的命理解释条目，包括宫位角色、星曜角色、四化类型边界、四化彼此合参边界、具体星曜被四化标记的角色边界、星曜四化同宫合参基础边界和十二宫四化落宫边界，让报告正文引用受控解释，而不是直接散写断语。
