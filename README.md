...
生产环境可以不把密钥直接写进 `.env`。`ZIWEI_RUNTIME_SECRETS_FILE` 可指向一个 JSON object，支持 `ZIWEI_API_CREDENTIALS`、`ZIWEI_API_TOKEN`、`ZIWEI_LLM_API_KEY`、`ZIWEI_LLM_ENDPOINT`、`ZIWEI_LLM_MODEL` 等运行时键；也可用 `ZIWEI_API_CREDENTIALS_FILE`、`ZIWEI_API_TOKEN_FILE`、`ZIWEI_LLM_API_KEY_FILE` 分别指向单项 secret 文件。直接环境变量优先于文件内容；secret 文件无法读取、JSON 不合法或包含未支持键时，运行时校验和部署校验会 fail closed。
...
发布前可以运行 `npm run validate:release`。它会串联全量测试、知识库校验、运行时配置校验、部署 smoke、`.env.example` 部署校验和 `git diff --check`。GitHub Actions 会在 push/PR 上执行同一条 release gate，并额外验证 Compose 模板和 Docker build。
...
- `src/runtimeEnv.js`: 解析运行时 secret 文件，把 mounted secret 或 JSON 配置汇合成服务、校验和 smoke 使用的 resolved env。
...
- `src/validateRelease.js`: 发布前总门禁，串联测试、知识库、运行时、部署、示例环境和 diff 检查。
