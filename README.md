...
当前阶段实现了出生资料采集、历法转换、命宫/身宫计算、五行局计算、十四主星基础落宫，左辅、右弼、天刑、天姚、天月、天巫等月系辅星，三台、八座等日系辅星，禄存、擎羊、陀罗、天魁、天钺、天官、天福、天厨等年干星曜，截空等空曜，火星、铃星等基础煞曜，生年四化，大限年龄段，以及按分析日期定位当前大限。排盘流程已经抽成可复用的 `chartBuilder` 服务，并加入命理师 agent 外壳，用“分析上下文 -> 报告规划 -> 正文草稿 -> 报告审计 -> 用户报告发布”的流程组织命盘证据、分析重点、当前能力边界和输出安全检查。事业、财富、婚姻和当前阶段运势已支持结构性底稿，但仍不能推具体年份事件、应期、吉凶、婚恋结果、职位结果或财富结果。后续会在此基础上加入资料检索、命理分析和完整报告生成。
...
- `src/agent/knowledgeSnippetCatalog.js`: 定义外部书籍、PDF、笔记和知识库片段的检索接口；当前只登记待录入资料库，不把未录入内容作为报告依据。
