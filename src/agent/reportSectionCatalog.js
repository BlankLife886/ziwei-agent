...
const SECTION_DEFINITIONS = {
  ...
  "star-balance": {
    ...
    writingPrompt: "先做星曜类别统计，再提醒读者当前缺少事件触发规则，不能过度推演。",
...
function buildCurrentStageInterpretationRefs(evidenceItems) {
  return [
    ...
    ...getFourTransformationStarRoleInterpretationRefs(evidenceItems),
    ...getFourTransformationStarPalaceInterpretationRefs(evidenceItems),
...
function getFourTransformationStarPalaceInterpretationRefs(evidenceItems) {
  const transformations = evidenceItems.flatMap((item) => {
    return item.metadata?.transformations ?? [];
  });

  return findFourTransformationStarPalaceInterpretationRefs(transformations);
}

function getFourTransformationStarRoleInterpretationRefs(evidenceItems) {
  const transformations = evidenceItems.flatMap((item) => {
    return item.metadata?.transformations ?? [];
  });
...
function getTimingTriggerInterpretationRefs(evidenceItems) {
  const hasTimingTriggerCandidates = evidenceItems.some((item) => {
    return item.metadata?.timingTriggerCandidates?.length > 0;
  });

  return hasTimingTriggerCandidates
    ? [INTERPRETATION_IDS.TIMING_TRIGGER_CANDIDATE_ONLY]
    : [];
}

function getTimingCombinationInterpretationRefs(evidenceItems) {
  const hasTimingCombinationVerifications = evidenceItems.some((item) => {
    return item.metadata?.timingCombinationVerifications?.length > 0;
  });

  return hasTimingCombinationVerifications
    ? [INTERPRETATION_IDS.TIMING_COMBINATION_VERIFIED_ONLY]
    : [];
}

function getTimingCombinationThemeInterpretationRefs(evidenceItems) {
  const hasTimingCombinationThemes = evidenceItems.some((item) => {
