  ...
  },
  {
    ...
    evaluate: ({ pipelineResult }) => {
      ...
      if (
        ...
        return partial("已建立报告生成器合同、provider 选择边界、通用 Tool Runtime、同步/异步 provider 链路、确定性 provider、通用外部 HTTP provider 适配器、超时、重试、响应大小限制、脱敏诊断、结构化恢复计划、知识记忆 manifest、本地稀疏向量检索、CLI 入口、HTTP API 入口、OpenAPI 合同入口、轻量 Web UI、多凭证 scoped bearer 鉴权、credential 禁用/生效/过期控制、托管密钥命令桥接、secret 文件载入、请求大小限制、请求追踪、结构化观测、release/build 元数据、liveness/readiness 探针、readiness draining、内存限流、可选文件持久化配额、运行时配置校验、部署校验、发布总门禁、机器可读 release summary、CI 工作流、API smoke 校验、运维手册、Dockerfile、Compose/Kubernetes 模板和 Cloudflare 真实部署验证；后续仍需补齐用户会话鉴权、外部 embedding/vector DB 和产品化报告体验。", 0.99);
      }
