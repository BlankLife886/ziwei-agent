...
async function main() {
  ...
  if (chart.lifePalace && chart.bodyPalace) {
    ...
    if (chart.starAnchors?.kuiYue) {
      const kuiYueText = Object.entries(chart.starAnchors.kuiYue)
        .filter(([key]) => key !== "yearStem")
        .map(([star, branch]) => `${star}${branch}`)
        .join("、");
      console.log(`魁钺星曜：${kuiYueText}`);
    }
    if (chart.starAnchors?.tianGuanFu) {
      const tianGuanFuText = Object.entries(chart.starAnchors.tianGuanFu)
        .filter(([key]) => key !== "yearStem")
        .map(([star, branch]) => `${star}${branch}`)
        .join("、");
      console.log(`官福星曜：${tianGuanFuText}`);
    }
    if (chart.starAnchors?.tianChu) {
      const tianChuText = Object.entries(chart.starAnchors.tianChu)
        .filter(([key]) => key !== "yearStem")
        .map(([star, branch]) => `${star}${branch}`)
        .join("、");
      console.log(`天厨星曜：${tianChuText}`);
    }
    if (chart.starAnchors?.jieKong) {
      const jieKongText = Object.entries(chart.starAnchors.jieKong)
        .filter(([key]) => key !== "yearStem")
        .map(([strength, branch]) => `${strength}${branch}`)
        .join("、");
      console.log(`截空星曜：${jieKongText}`);
    }
