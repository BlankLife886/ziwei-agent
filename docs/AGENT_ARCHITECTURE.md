...
知识片段录入遵循候选材料、draft、verified 三段流程：扫描件 OCR、PDF 摘录或研读笔记先作为候选片段进入 `knowledgeSnippetIngestor` 标准化；字段完整后仍保持 draft；只有经过人工复核并晋升为 verified 的片段，才允许被 `reportPlanner` 检索并进入报告引用链。实际操作入口是 `src/manageKnowledgeSnippets.js`：`draft` 负责标准化候选摘录，`promote` 负责执行 verified 晋升审计，`append` 负责把 verified 片段追加到 JSON store 并在写入前执行全库审计。
