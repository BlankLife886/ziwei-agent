...
- 阶段运势：在提供 `analysis_date` 后定位当前大限，合看大限四化、流年骨架、流年四化和流月骨架，并形成安全触发观察点和当前阶段底稿。
...
- 大模型生成尚未接入。
- 大限四化、流年骨架、流年四化和流月骨架已接入，但深层组合验证仍然很少。
- 宫位、星曜、四化、运限的组合解释仍然需要扩充。
...
3. 增加组合验证：让阶段运势从安全观察点进入更细的多层交叉分析。
