...
系统级架构文本和 Markdown Mermaid 架构图见 `docs/ZIWEI_AGENT_SYSTEM_ARCHITECTURE.md`。本文档侧重当前实现与审计边界，系统架构文档侧重总体分层、执行时序、状态机、知识库演进和 Cloudflare 部署拓扑。
...
- `openApiDocument`：输出当前 HTTP API 的 OpenAPI 3.1 合同，只描述 `/health`、`/ready` 和 `POST /v1/reports` 的真实边界，不承诺未实现的报告能力。
...
- `introduction`：报告开篇。
- `brief`：面向用户的报告摘要，包含报告模式、命盘摘要、章节路径、章节计数和交付边界。
...
- `appendix`：可追溯附录，集中列出本报告使用的 evidence、references、sources、knowledgeSnippets、interpretations 和 traceability 汇总，供 Web UI、CLI、PDF 导出或后续大模型复核使用。
- `closing`：报告收束和边界提示。
- `audit`：发布时使用的审计结果。
...
- 事业专题：官禄宫三方四正，保守描述职责承担、主体基础、资源承接和合作牵动；当生年四化、当前大限四化或流年四化落入官禄宫时，专题章节会追加四化合参边界，但仍不推职位或阶段结果。
- 财富专题：财帛宫三方四正，保守描述资源经营、主体基础、事业承接和内在取舍；当生年四化、当前大限四化或流年四化落入财帛宫时，专题章节会追加四化合参边界，但仍不推金额或资源结果。
- 婚姻专题：夫妻宫三方四正，保守描述关系互动、外部相处、现实承担和内在感受；当生年四化、当前大限四化或流年四化落入夫妻宫时，专题章节会追加四化合参边界，但仍不推分合或婚恋结果。
- 阶段运势：在提供 `analysis_date` 后定位当前大限，合看大限四化、流年骨架、流年四化和流月骨架，并形成安全触发观察点、组合验证主题、组合主题解释、跨宫跨限运关系和当前阶段底稿；化禄、化权、化科、化忌只作为资源流动、推动压力、秩序修饰和牵挂约束等结构信号，四化彼此合参只说明资源、推动、秩序和约束之间的结构关系，具体星曜被四化标记时先按星曜角色解释，再进入星曜四化同宫合参基础层，要求先看星曜在目标宫位承担的角色，再把四化作为加权信号，十二宫四化落宫只作为对应人生领域被牵引的提示，不写成结果断语。
...
- 有本地参考目录和解释目录；性格画像的命宫星曜解释目录已覆盖十四主星和常见辅煞，四化类型解释已覆盖化禄、化权、化科、化忌，四化彼此合参已覆盖禄权、禄科、禄忌、权科、权忌、科忌，具体星曜被四化标记的角色边界已覆盖紫微、天机、太阳、武曲、天同、廉贞、太阴、贪狼、巨门、天梁、破军、文昌、文曲、左辅、右弼，星曜四化同宫合参基础边界已覆盖化禄、化权、化科、化忌，四化落宫解释已覆盖十二宫，婚姻、财富、事业专题已能在本宫被生年四化、当前大限四化或流年四化命中时引用专题合参边界，命宫三方四正、生年四化、专题章节和当前阶段章节可按结构化证据引用受控解释条目。
...
- 有最终用户报告可追溯附录，能把每份报告实际使用的证据、规则、来源、知识片段和解释条目集中交付，便于审计、复核、PDF 化和后续大模型二次整理。
...
- 外部知识库片段 schema、检索和可用性审计已建立，示例库已有 20 条本地审校框架样本和专题知识笔记；书籍/PDF内容尚未全量结构化录入。
...
- 大限四化、流年骨架、流年四化、流月骨架、四化类型解释、四化彼此合参、具体星曜被四化标记的角色边界、星曜四化同宫合参基础边界、十二宫四化落宫解释、组合验证底座、组合主题解释、跨宫跨限运关系解释和专题细分任务单已接入，但细分组合规则和文献支撑仍然很少。
- 宫位、星曜、四化、运限之间的深层专题化解释仍然需要扩充，尤其是跨宫组合、具体星曜在具体宫位遇四化后的专题细分、同宫多信号叠加和文献支撑。
- 因果、前世今生等主题只有目标登记，还不能生成深入报告。
- 当前报告已有摘要、章节正文、交付边界和可追溯附录，但正文表达仍偏底稿，需要后续加入更完整的分层章节、建议模块、PDF/长报告排版和更细的风险边界。

## 下一步优先级

建议继续按“先底层、再能力、后表达”的顺序推进：

1. 扩充解释目录：继续为跨宫组合、具体星曜与四化同宫合参、同宫多信号叠加和专题细分增加受控解释条目。
