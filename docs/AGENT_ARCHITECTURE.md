...
当前项目还没有进入“完整命运报告”阶段。现在的核心产出是可审计的报告草稿底稿：它能说明用了哪些命盘证据、引用了哪些本地规则、哪些解释条目参与了正文生成，以及哪些结论不能写。
...
- 阶段运势：在提供 `analysis_date` 后定位当前大限，并形成当前阶段底稿。
...
2. 建立知识库引用层：把文档、书籍、PDF 摘录映射为可检索的 reference。
