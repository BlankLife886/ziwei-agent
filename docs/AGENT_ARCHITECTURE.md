...
CLI 和 HTTP API 都必须进入这条链路。`ziweiApiHandler` 只负责接收 `profile`、`query` 或显式 `queryIntent`，然后调用 `buildChart` 和 `runZiweiPipelineAsync`；它不会直接调用报告 composer，也不会跳过审计发布门禁。`server`、`apiCredentials`、`apiObservability`、`apiQuotaStore` 和 `apiRateLimiter` 只负责服务入口治理，例如 scoped 鉴权、请求追踪、脱敏日志、限流、配额窗口持久化、请求体大小限制和响应封装，不负责命理解释。
...
- 报告生成器合同、provider 选择边界、确定性 provider、异步 provider 链路、通用外部 HTTP provider 适配器、超时、重试、响应大小限制、脱敏诊断、CLI 入口和 HTTP API 入口已建立；API 已有多凭证 scoped bearer 鉴权、请求大小限制、请求追踪、结构化观测、内存限流和可选文件持久化配额，但 UI、密钥生命周期管理和生产部署尚未接入。
