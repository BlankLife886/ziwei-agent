...
test("zi wei star group is placed into matching palaces", () => {
  const result = buildSampleChartWithZiWeiGroup();
...
test("all fourteen main stars match the reference table", () => {
  for (const [ziWeiBranch, expected] of Object.entries(MAIN_STAR_TABLE)) {
...
test("tian fu star group is placed into matching palaces", () => {
  const result = applyTianFuStarGroup(buildSampleChartWithZiWeiGroup());
  const ziWeiPalace = result.palaces.find((palace) => {
    return palace.branch === "未";
  });
...
function buildSampleChartWithZiWeiGroup() {
  const chart = createSampleChart();
  const chartWithPalaces = applyLifeAndBodyPalaces(chart, {
    lunarMonth: 4,
    chineseHour: "子时"
  ...
  return applyZiWeiStarGroup(chartWithZiWei);
}

const MAIN_STAR_TABLE = {
  子: {
    紫微: "子",
    天机: "亥",
    太阳: "酉",
